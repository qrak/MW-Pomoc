...
function showli($directory) { 
 ...
 if ($countdir != "0") { 
  ...
  while ($count2 < $countdir) { 
	 echo ('<ul><a href="skrypt_zbieranina/'.$dirname[$count2].'"">'.$dirname[$count2].'</a></ul>');
 ...
 if ($countfile != "0") { sort($filename) ;$count2="0"; 
  while ($count2 < $countfile) { 
	 echo ('<ul><a href="skrypt_zbieranina/'.$filename[$count2].'" TARGET="_blank">'.$filename[$count2].'</a></ul>');
   $count2++; 
  } 
 } 
